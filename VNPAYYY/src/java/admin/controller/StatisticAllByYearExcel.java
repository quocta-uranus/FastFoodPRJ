 ...
 * @author Asus
 */
public class StatisticAllByYearExcel extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet StatisticAllByYearExcel</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet StatisticAllByYearExcel at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int sumByStore = orderDAO.sumOrderByYear(year);
            ...
            try ( XSSFWorkbook workbook = new XSSFWorkbook();  OutputStream out = response.getOutputStream()) {
                XSSFSheet sheet = workbook.createSheet("Doanh_Thu_Nam_" + year + "_Cua_He_Thong");
                ...
                response.setHeader("Content-Disposition", "attachment; filename=Doanh_thu_nam_" + year + "_cua_he_thong.xlsx");

                workbook.write(out);
            }
        } catch (Exception ex) {
            Logger.getLogger(StatisticAllByYearExcel.class.getName()).log(Level.SEVERE, null, ex);
