 ...
 * @author Asus
 */
public class StatisticByStoreByYearExcel extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet StatisticByStoreByYearExcel</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet StatisticByStoreByYearExcel at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int sumByStore = orderDAO.sumOrderOfStoreByYear(year, store_id);
            List<Order> listOrder = orderDAO.getOrderOfStoreByYear(year, store_id);

            // Tạo một tệp Excel bằng Apache POI
            try ( XSSFWorkbook workbook = new XSSFWorkbook();  OutputStream out = response.getOutputStream()) {
                XSSFSheet sheet = workbook.createSheet("Doanh_Thu_Theo_Nam");
                ...
                response.setHeader("Content-Disposition", "attachment; filename=Doanh_thu_theo_nam_cua" + storeName + ".xlsx");

                workbook.write(out);
            }
        } catch (Exception ex) {
            Logger.getLogger(StatisticByStoreByYearExcel.class.getName()).log(Level.SEVERE, null, ex);
